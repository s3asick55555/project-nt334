...
# Summary
A Python-based clipboard hijacker that detects Bitcoin addresses, replaces clipboard content with a hardcoded BTC address, self‑replicates to %APPDATA%, registers itself in the Run key for persistence, and then self‑destructs. It demonstrates typical ransomware‑style clipboard theft and persistence mechanisms.
