...
class Py_Stack_Call(interfaces.plugins.PluginInterface):
    ...
    def _collect_data(self, processes):
        ...
        collected_data = self.parse_interpreters(
            self.context, curr_layer, interpreters_head, python_table_name, process)
        return collected_data

    def parse_interpreters(self, context, layer_name, interpreters_head_ptr, python_table_name, process):
        """
        ...
        for interp in interpreters:
            ...
            while current_tstate and current_tstate.vol.offset != 0:
                ...
                if frame_ptr != 0:
                    ...
                    disasm_file = f"chain_thread_{indx}_{process_id}_disassembly.txt" if self.config.get('dump', False) else "disabled"
                    if self.config.get('dump', False):
                        # Save the disassembly to a file if requested 
                        self.save_chain_disassembly(frames, process_id, indx)
                    ...
                    collected_data.append((process_id, indx, num_frames, print_full_chain_value, disasm_file))
    ...
    def save_chain_disassembly(self, frames, process_id, thread_id):
        """
        ...
        filename = f"chain_threads_{process_id}_{thread_id}_disassembly.txt"
        try:
            with open(filename, 'w') as f:
                f.write(f"Disassembly for Thread {thread_id} (PID {process_id}) Call Chain\n")
                f.write("=" * 50 + "\n\n")
