...
class Py_Class(interfaces.plugins.PluginInterface):  
    ...
    Usage:
    python3 vol.py -f "path/to/mem dump" linux.py_class.Py_Class --pid=<process_id>
    ...
    def get_requirements(cls):
        ...
        - ELFSymbolFinder (elf_symbols plugin) to locate _PyRuntime symbol
        ...
        for obj_info in objects:
            ...
            if obj_type == 'module':
                ...
                if module_dict:
                   ...
                   collected_data=self.recursive_process(module_dict,generation,task,python_table_name,collected_data)
                  
                   # Add a separator line"""
                   collected_data.append((int(task.pid), generation, "-----------------", "--------", "-------------------", "-----------------------"))
            ...
            # Get all the class isntances that are represented as PyDictObject
                   ...
                   try:
                           ...
                           collected_data.append((int(task.pid), generation, obj_type," ", obj_address, value))
                           collected_data.append((int(task.pid), generation, "-----------------", "--------", "-------------------", "-----------------------"))
    ...
    def recursive_process(self,obj_dict,generation,task,python_table_name,collected_data):
        """
        ...
        for k, v in obj_dict.items():
                       ...
                       collected_data.append((int(task.pid), generation, vtype, k, v_address, vv))
        collected_data.append((int(task.pid), generation, "---------------------", "---------------------", "---------------------", "---------------------"))
        ...
        for t in types:
           ...
           self.recursive_process(class_dict,generation,task,python_table_name,collected_data)
        ...
        for i in range(GC_GENERATIONS):
            ...
            while current_offset != PyGC_Head:
                ...
                try:
                    ...
                    #--------------------------------------------------------
                    # Note: we should first identify the classes in the main file (__main__) and 
