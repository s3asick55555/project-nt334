...
class Py_Stack_Var(interfaces.plugins.PluginInterface):
    ...
    def _collect_data(self, tasks):
        ...
        collected_data = self.parse_interpreters(
            self.context, curr_layer, interpreters_head, python_table_name, target_frame,task
        )
        return collected_data

    
    def parse_interpreters(self, context, layer_name, interpreters_head_ptr, python_table_name, target_frame, task):
      """
        ...
        while current_tstate and current_tstate.vol.offset != 0:
            ...
            if frame_ptr != 0:
                ...
                for fun_name, var_name, var_value in local_vars:
                    collected_data.append((task_id, fun_name, var_name, var_value))
